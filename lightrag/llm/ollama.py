...
async def _ollama_model_if_cache(
    ...
    **kwargs,
    ...
    try:
        ...
        logger.debug(f"start call ollama model:{model}, input: {messages[-1]['content']}")
        response = await ollama_client.chat(model=model, messages=messages, **kwargs, keep_alive=-1)
        if stream:
            ...
            async def inner():
                ...
                try:
                    async for chunk in response:
                        ...
                        if start_response:
                            logger.debug(f"Start Stream response:{type(chunk)}, {chunk}")
                            start_response = False
                        if '<think>' in content:
                            logger.info(f"Start Stream response with think tag: {type(chunk)}, {chunk}")
                            if os.environ.get("SUMMARY_LANGUAGE") == 'Chinese':
                                yield "\n\n> *思考过程：*\n> "
                            else:
                                yield "\n\n> *Thinking Process：*\n> "
...
async def ollama_embed(texts: list[str], embed_model, **kwargs) -> np.ndarray:
    ...
    ollama_client = ollama.AsyncClient(host=host, timeout=timeout, headers=headers)
    try:
        logger.debug(f"start call embed_model:{embed_model}, input: {texts}")
        data = await ollama_client.embed(model=embed_model, input=texts, keep_alive=-1)
