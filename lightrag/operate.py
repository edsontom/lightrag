...
async def _build_query_context(
    ...
    """
    Build a compact query context for LLM consumption with aggressive token optimization:
    - Early deduplication and round-robin merging to avoid bias
    - Minified JSON (no spaces) to reduce tokens
    - Drop noisy/low-signal fields (ids, timestamps)
    - Trim long descriptions by token budget
    - Compact file_path to at most 1-3 basenames
    - Dynamic token budgeting for text chunks
    - Normalize chunk content whitespace (low loss, high gain)
    Note: External function signature and returned structure (3 sections) preserved.
    """
    ...
    # Round-robin merge with strict dedup
    ...
    for i in range(max_len):
        if i < len(local_relations):
            r = local_relations[i]
            rel_key = tuple(sorted(r["src_tgt"])) if "src_tgt" in r else tuple(
                sorted([r.get("src_id"), r.get("tgt_id")])
            )
    ...
    # Helpers
    def _compact_sources(raw_file_path: str, keep: int = 1) -> str:
        """Keep at most 'keep' distinct basenames from file_path; fallback to 'unknown_source'"""
        raw = raw_file_path or "unknown_source"
        if raw == "unknown_source":
            return raw
        parts = split_string_by_multi_markers(raw, [GRAPH_FIELD_SEP])
        basenames, seen_fp = [], set()
        for p in parts:
            b = os.path.basename(p) if p else ""
            if b and b not in seen_fp:
                seen_fp.add(b)
                basenames.append(b)
                if len(basenames) >= keep:
                    break
        return ";".join(basenames) if basenames else "unknown_source"

    def _strip_ws(text: str) -> str:
        """Trim excessive whitespace while keeping single newlines for readability"""
        if not text:
            return text
        # collapse >2 newlines to single newline and trim spaces around
        return re.sub(r"[ \t\f\v]+", " ", re.sub(r"\n{2,}", "\n", text)).strip()

    # Build compact entities context (no id/created_at)
    entities_context = []
    for n in final_entities:
        entities_context.append(
            {
                "entity": n.get("entity_name", "UNKNOWN"),
                "type": n.get("entity_type", "UNKNOWN"),
                "description": n.get("description", "UNKNOWN"),
                "file_path": _compact_sources(n.get("file_path"), keep=1),
            }
        )

    # Build compact relations context (no id/created_at)
    relations_context = []
    for e in final_relations:
        if "src_tgt" in e:
        ...
        relations_context.append(
            {
                ...
                "file_path": _compact_sources(e.get("file_path"), keep=1),
            }
        )
    ...
    # Pre-trim descriptions by token
    max_entity_desc_tokens = text_chunks_db.global_config.get("max_entity_desc_tokens", 72)
    max_relation_desc_tokens = text_chunks_db.global_config.get("max_relation_desc_tokens", 60)

    def _truncate_to_token(text: str, limit: int) -> str:
        if not text:
            return text
        # quick strip to reduce waste before tokenization
        compact = _strip_ws(text)
        ids = tokenizer.encode(compact)
        if len(ids) <= limit:
            return compact
        return tokenizer.decode(ids[:limit]).strip()

    if entities_context:
        for e in entities_context:
            e["description"] = _truncate_to_token(e.get("description", ""), max_entity_desc_tokens)
            if e.get("file_path"):
                e["file_path"] = e["file_path"].replace(GRAPH_FIELD_SEP, ";")
        # Minify JSON by key function and truncate list by tokens
    ...
    if relations_context:
        for r in relations_context:
            r["description"] = _truncate_to_token(r.get("description", ""), max_relation_desc_tokens)
            if r.get("file_path"):
                r["file_path"] = r["file_path"].replace(GRAPH_FIELD_SEP, ";")
    ...
    # Filter original nodes/edges by truncated context
    final_node_datas: list[dict] = []
    ...
    # Retrieve text chunks (entity first, then relation)
    if final_node_datas:
        entity_chunks = await _find_most_related_text_unit_from_entities(
            final_node_datas, query_param, text_chunks_db, knowledge_graph_inst
        )
    if final_edge_datas:
        relation_chunks = await _find_related_text_unit_from_relationships(
            final_edge_datas, query_param, text_chunks_db, entity_chunks
        )

    # Round-robin merge chunks with dedup
        ...
        if i < len(vector_chunks):
            ...
            if cid and cid not in seen_chunk_ids:
                ...
                merged_chunks.append(
                    {"content": c["content"], "file_path": c.get("file_path", "unknown_source")}
                )
    ...
    logger.debug(f"Round-robin merged total chunks from {origin_len} to {len(merged_chunks)}")

    # Apply token processing to merged chunks
    text_units_context: list[dict] = []
    if merged_chunks:
        # Minified strings for token budgeting
        entities_str_min = json.dumps(entities_context, ensure_ascii=False, separators=(",", ":"))
        relations_str_min = json.dumps(relations_context, ensure_ascii=False, separators=(",", ":"))

        # Estimate overhead minimally (no pretty templates)
        overhead_tokens = 0
        if query_param.conversation_history:
            history_context = get_conversation_turns(
                query_param.conversation_history, query_param.history_turns
            )
            overhead_tokens += len(tokenizer.encode(history_context))
        ...
        sample_sys_prompt = sys_prompt_template.format(
            history="", context_data="", response_type=response_type, user_prompt=user_prompt
        )
        overhead_tokens += len(tokenizer.encode(sample_sys_prompt))
        overhead_tokens += len(tokenizer.encode(query))

        # KG sections minified (headers + backticks add small overhead)
        kg_headers_overhead = len(
            tokenizer.encode("-----Entities(KG)-----```json````----Relationships(KG)-----```json```")
        )
        kg_context_tokens = (
                len(tokenizer.encode(entities_str_min)) + len(tokenizer.encode(relations_str_min)) + kg_headers_overhead
        )

        # Smaller safety buffer
        buffer_tokens = 64
        used_tokens = overhead_tokens + kg_context_tokens + buffer_tokens
        available_chunk_tokens = max(max_total_tokens - used_tokens, 0)

        logger.debug(
            f"Token allocation - Total:{max_total_tokens}, Overhead:{overhead_tokens}, "
            f"KG:{kg_context_tokens}, Buffer:{buffer_tokens}, Available chunks:{available_chunk_tokens}"
        )

        # Pre-shrink chunk content whitespace before truncation
        for c in merged_chunks:
            c["content"] = _strip_ws(c.get("content", ""))
        ...
        for i, chunk in enumerate(truncated_chunks):
            text_units_context.append(
                {
                    # drop id to save tokens
                    "content": chunk["content"],
                    "file_path": _compact_sources(chunk.get("file_path"), keep=1),
                }
            )

        logger.debug(
            f"Final chunk processing: {len(merged_chunks)} -> {len(text_units_context)} "
            f"(available chunk tokens: {available_chunk_tokens})"
        )
    ...
    # If nothing from KG, skip LLM step
    if not entities_context and not relations_context:
        return None

    # Final minified JSON payload to reduce tokens significantly
    entities_str = json.dumps(entities_context, ensure_ascii=False, separators=(",", ":"))
    relations_str = json.dumps(relations_context, ensure_ascii=False, separators=(",", ":"))
    text_units_str = json.dumps(text_units_context, ensure_ascii=False, separators=(",", ":"))

    # Keep the 3-section format for downstream prompts, but avoid extra spaces/newlines
    result = (
        "-----Entities(KG)-----\n\n```json\n"
        f"{entities_str}\n```\n\n"
        "-----Relationships(KG)-----\n\n```json\n"
